...
void loop() {
  ...
  for (int i = 0; i < NUM_SLOTS; i++) {
    ...
    String currentDb = Database.get<String>(aClient, path);

    // IF SLOT IS DISABLED IGNORE SENSOR
    if (currentDb == "unavailable") {
      continue;
    }
